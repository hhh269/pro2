...
test('tree', () => {
  ...
  //点击tree效果
  moduleManage.find('tbody tr').at(0).find('span').at(1).simulate('click')
  expect(moduleManage.find('tbody tr').length).toBe(
    fetchTreeList.data.list.length + fetchTreeList.data.list[0].children.length
  )
  //状态以上线的类名
  expect(moduleManage.find('.success').at(0).text()).toBe('已上线')
})
test('cancelButton', () => {
...
test('addButton/editButton', () => {
  ...
  //权限按钮点击
  moduleManage.find('tbody tr').at(0).find('a').at(3).simulate('click')
